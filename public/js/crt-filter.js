var CrtY = 0;
window.crtOn = localStorage.getItem('crtOn');
if (crtOn === null) {
    crtOn = false;
}
let bool = crtOn === "true";
...
window.addEventListener('load', CRTFilter, false);

/*No public urls avalible*/
/**
 * crt-filter.js
 * No public urls avalible
 * 
 * Copyright 2024 of Elijah Ledlow
...
window.checkGLSupport = webGLSupport();
if (checkGLSupport) {
    console.log("Your device/browser supports WebGL");
}else{
    console.warn("Your device/browser does not support WebGL. Please upgrade to a better browser or try a different device to use the CRT Filter feature.");
    alert("Your device/browser does not support WebGL. Please upgrade to a better browser or try a different device to use the CRT Filter feature.");
}
...
function CRTFilter() {
    ...
    try {
        ...
        return alert(`This specific script of WebGL is unsupported. Returned: "${e}"`);
    }


    //var gl = glcanvas.gl; // Get the WebGL context
    ...
    //texture.minFilter = gl.NEAREST; // disable image smoothing
    //texture.magFilter = gl.NEAREST; // disable image smoothing
    ...
    if (!requestAnimationFrame) {
        glcanvas.style.imageRendering = 'none';
        console.log("Your device/browser does not support requestAnimationFrame. Therefore using WebGL is not advised.");
        // Not as smooth as requestAnimationFrame
        // But is more supported
        setInterval(function () {
            ...
            if (crtOn) {
                ...
                CrtY = CrtY + 0.1;
                srcctx.drawImage(lines, 0, CrtY, w, h);
        ...
        console.log("Your device/browser supports requestAnimationFrame. Using this webGL feature will run smoothly.");
        // Is smother than setInterval
        // But not as supported
        function draw() {
            if (crtOn && allowCRTFilter) {
                source.style.display = 'none';
                glcanvas.style.display = '';
            } else {
                glcanvas.style.display = 'none';
            ...
            requestAnimationFrame(draw, drawButtons());
        }
